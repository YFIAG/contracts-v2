 ...
 */
contract Stub_CurveFi_Gauge is ICurveFi_Gauge, Context {
    ...
    constructor(address lp_addr, address _minter) public {
        __minter = _minter;
        __lp_token = lp_addr;
    ...
    function deposit(uint256 _value) override public {
    ...
    function withdraw(uint256 _value) public override {
        _checkpoint(_msgSender());
    ...
    function claimable_tokens(address addr) external override returns (uint256) {
        _checkpoint(addr);
    ...
    function _checkpoint(address addr) internal {
        ...
        __integrate_fraction[addr] += _working_balance * 
                                (_integrate_inv_supply - integrate_inv_supply_of[addr]) / 10 ** 18;
    ...
    function crv_token() public override view returns(address) {
        return __crv_token;
    }

    function lp_token() public override view returns(address) {
        return __lp_token;
    }

    function integrate_fraction(address _for) public override view returns(uint256) {
