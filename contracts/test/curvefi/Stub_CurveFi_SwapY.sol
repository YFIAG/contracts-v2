 ...
 */
contract Stub_CurveFi_SwapY is ICurveFi_SwapY, Context {
    ...
    constructor(address[N_COINS] memory _coins,
    ...
    function add_liquidity(uint256[N_COINS] memory amounts, uint256 min_mint_amount) public override {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            IERC20(__coins[i]).transferFrom(msg.sender, address(this), amounts[i]);
        }

        ERC20Mock(__token).mint(_msgSender(), mint_amount);
    }
    ...
    function remove_liquidity (uint256 _amount, uint256[N_COINS] memory min_amounts) public override {
        ...
        for (uint256 i=0; i < __coins.length; i++){
            ...
            IERC20(__coins[i]).transfer(_msgSender(), amounts[i]);
        }
        ERC20Mock(__token).burnFrom(_msgSender(), _amount);
    }
    ...
    function remove_liquidity_imbalance(uint256[N_COINS] memory amounts, uint256 max_burn_amount) public override {
        ...
        ERC20Mock(__token).burnFrom(_msgSender(), token_amount);
    }
...
//            D1: uint256 = self.get_D_mem(rates, balances)
 //           mint_amount = D1;
    ...
    function calc_token_amount(uint256[N_COINS] memory amounts, bool deposit) public override view returns(uint256) {
...
//            diff = D0 - D2;
    ...
    function normalize(address coin, uint256 amount) internal view returns(uint256) {
        uint8 decimals = ERC20Mock(coin).decimals();
        if (decimals == 18) {
            return amount;
        } else if (decimals > 18) {
    ...
    function balances(int128 i) public override view returns(uint256) {
    ...
    function coins(int128 i) public override view returns (address) {
