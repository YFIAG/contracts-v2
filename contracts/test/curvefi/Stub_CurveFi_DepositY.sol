 ...
 */
contract Stub_CurveFi_DepositY is ICurveFi_DepositY, Context {
    ...
    function add_liquidity (uint256[N_COINS] memory uamounts, uint256 min_mint_amount) public override {
        ...
        for (uint256 i = 0; i < uamounts.length; i++) {
            ...
            amounts[i] = IERC20(__coins[i]).balanceOf(address(this));
    ...
    function underlying_coins(int128 i) public override view returns (address) {
        return __underlying[uint256(i)];
    }

    function underlying_coins() public override view returns (address[N_COINS] memory) {
        return __underlying;
    }

    function curve() public override view returns (address) {
        return __curve;
    }
    ...
    function _send_all(address _addr, uint256[N_COINS] memory min_uamounts) internal {
        for (uint256 i = 0; i < N_COINS; i++) {
            ...
            uint256 _balance = IERC20(_coin).balanceOf(address(this));
    ...
    function remove_liquidity_one_coin(uint256 _token_amount, int128 i, uint256 min_uamount) external override {
        int a;
    }
