...
const CRVStrategyWRenBTC = artifacts.require('CRVStrategyWRenBTC');
...
contract('CRVStrategyYCRV test', accounts => {
    ...
    describe('Check constructor', () => {
        ...
        it(`Constructor will revert if vault will not support underlying token`, async () => {
            ...
            await expectRevert( CRVStrategyWRenBTC.new(vault.address, token.address, controller.address, governance,strategist,curveGauge.address,curveMinter.address, crvToken.address, curveSwap.address, constants.ZERO_ADDRESS,dai.address,ydai.address,uniswap.address), "vault does not support underlying");
        });
    });
    ...
    async function deployStrategy() {
        ...
        strategy = await CRVStrategyWRenBTC.new(vault.address, token.address, controller.address, governance,strategist,curveGauge.address,curveMinter.address, crvToken.address, curveSwap.address, constants.ZERO_ADDRESS,dai.address,ydai.address,uniswap.address);
